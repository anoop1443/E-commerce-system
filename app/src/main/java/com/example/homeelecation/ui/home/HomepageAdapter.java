    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        switch (homepageModellist.get(position).getType()) {
            ...
            case HomepageModel.STRIP_AD_BANNER:
                ...
                String backgoundcolor = homepageModellist.get(position).getBackgoundcolor();
                ((StripAdViewHolder) viewHolder).StripAd(resource, backgoundcolor);
                return;
            case HomepageModel.HORIZONTAL_PRODUCT:
                String horizontalTitel = homepageModellist.get(position).getTitel();
                ...
                return;
            case HomepageModel.GRID_PRODUCT_VIEW:
                String griptitel = homepageModellist.get(position).getTitel();
            ...
            for (int x = 0; x < 4; x++) {
                ...
                if (!Title.equals("")) {
                    ...
                    gridLayout.getChildAt(x).setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent productDetails = new Intent(itemView.getContext(), ProductDeteilsActivity.class);
